...
| `skills-registry.json` | `read_only` | Derived catalog of installed skills, generated by the runtime indexer and injected via `{{skills-registry.json}}`. Runtime-owned while `skills.enabled` is true: agent writes are refused. Turning the subsystem off releases the hold back to `none` — see §5.1. |
...
path on first index. When `skills.enabled` is turned off the runtime MUST
release that hold (protection back to `none`), since it no longer maintains the
file. When `skills.enabled` is turned on the runtime MUST reindex at that
moment, so that neither an empty catalog nor an unadopted agent-authored
registry is injected on the following turn.
