...
| `skills-registry.json` | `read_only` | Derived catalog of installed skills, generated by the runtime indexer and injected via `{{skills-registry.json}}`. Runtime-owned: agent writes are refused. |
...
is installation; `skills-state.json` holds the mute list; no per-skill state
lives in `adf_config`. The only configuration is subsystem policy:
`skills.enabled` (and optional `skills.catalogs`).

When `skills.enabled` is true, a conforming runtime indexes `skills/*/SKILL.md`
into the derived `skills-registry.json` and injects that catalog into the system
prompt via the `{{skills-registry.json}}` placeholder. **Indexing and injection
are the only things the runtime does.** It MUST NOT execute skill text,
...
The indexer MUST bound and validate its catalog (name/directory agreement, file
size, entry count, serialized registry size), report rejected packages with a
reason rather than dropping them silently, keep disable state separate from
installed source, and write the generated registry outside `skills/` so its own
output does not retrigger indexing. `skills-registry.json` is runtime-owned:
the runtime SHOULD hold it at protection `read_only` and adopt a pre-existing
agent-authored registry at that path on first index. Mid-session catalog changes
MUST NOT rewrite the injected snapshot (that would invalidate prompt caching);
they are delivered as a keyed `loop_inject` that supersedes earlier catalogs,
and the snapshot refreshes at the next session reset (compaction / `loop_clear`).

When `skills.enabled` is false the runtime indexes nothing and the paths are
ordinary files. An agent MAY still maintain the same registry, state file,
instruction placeholder, and system-scope indexer lambdas entirely in agent
space using ordinary file, trigger, and code APIs. The canonical first-party
catalog is linked from ADF's default base prompt.
