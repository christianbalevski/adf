...
- ADF starts a display stack in that container (X server, the Openbox window manager, the tint2 panel, VNC, and a loopback-published noVNC endpoint). The Studio Computer tab embeds that noVNC page and shows the X display; it is not a second desktop session. Every window gets a close button and a panel task button, so the human can close or switch windows.
- The managed Chromium is supervised: if it exits, it is relaunched. Use `adf-browser stop` / `adf-browser resume` when a task needs it down (for example a profile swap).
