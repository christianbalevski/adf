...
**Egress (SSRF) guard:** loopback (`localhost`, `127.0.0.0/8`, `::1`) is reachable by default — except the local ADF daemon control API, which is never fetchable. `sys_fetch` blocks link-local and private-network destinations — `169.254.0.0/16` (incl. cloud metadata `169.254.169.254`), `10/8`, `172.16/12`, `192.168/16`, CGNAT `100.64/10`, and the unspecified address (`0.0.0.0`/`::`). The check runs on the **DNS-resolved** address (so a rebinding record pointing at a private IP is rejected) and re-runs on **every redirect hop** (a public URL that 302s to a private IP is stopped). The opt-in escape hatch for private/LAN destinations is `security.allow_local_fetch: true` in config. That flag is locked by default in the runtime, for every agent: a write is denied but surfaces as a protection request your principal can approve as a one-time override (see [sys_update_config](#sys_update_config)).
...
In `adf_shell` the same tool is the `state` command — `state idle`, or bare `state` to read the current one — which lets a yield be chained with the work before it in a single tool call. See [Agent States > Setting state from the shell](agent-states.md#setting-state-from-the-shell).
...
| Identity | `whoami`, `config`, `status`, `state`, `meta`, `env`, `export`, `pwd`, `date` |
