...
> **Implementation status (2026-09-01):** S1 + S2 are implemented on this branch through commit `e9835ba`. The runtime waves ran to `f466fae`; a UX / self-curation batch then landed on top (`23fa296` compact inter-loop cards → `319ed6a` `loop_manage` on+ungated + prompt teaching + user guide → `d051ec5` present-when-enabled registration → `9be9e6d` rail removal → `6be7003` agent-scope-only timer loop → `c2e82b3` held mid-stream deliveries → `27ab585` + `e9835ba` busy-target wake). **Read "Supersessions (2026-09-01)" below before this section** — that batch changed several statements made here and in §7.1/§7.2. `docs/design/agent-loops-review-findings.md` "Review 2" is the ledger for the findings it produced. Original wave detail (waves: `9ded83f` S1+config → `75d965f` tools/renderer/tests → `466b94c`+`b465e69` adversarial-review fixes → `2d1ba3b` runtime integration → `f466fae` runtime-review fixes). Six adversarial review passes ran; all confirmed findings fixed. `docs/design/agent-loops-wave3-brief.md` records the accumulated runtime requirements and accepted deviations (delete both notes before merge to main). Deltas from this plan as written: `loop_compact`/`loop_clear` default-on means host-enabled-and-unrestricted (restricted default-ons are not granted); side loops get `packages: []` (inheritance was worldly authority); per-loop model overrides are same-provider only (cross-provider → F3); `LoopInfo` has no `mode` (F3); config changes apply to side loops immediately, not at turn boundaries (revocations bite at the next model call). Known follow-up: several hand-rolled config fan-out sites (MCP tool sync, visibility toggles — see f466fae commit discussion) share the staleness class C2 fixed for the main save paths.
>
> **§7.1 superseded (2026-08-29, no-secrets):** the *hardwired essentials* are gone. `loop_send`/`loop_list` are ordinary config-declared tools — `enabled: true, visible: true` in `DEFAULT_TOOLS`, shown in the Tools card's Loop group, granted to a side loop only when the host has them enabled AND the loop's own `tools` allow-list names them (no union after the intersection), and the synthetic main-side declaration injection (`withLoopEssentialDeclarations`) is deleted. ~~Registration stays pool-gated, so a loop-less agent's model tool list is unchanged despite the declarations.~~ **Superseded 2026-09-01:** registration is *present-when-enabled* — see below. What survives of the old concept is `DEFAULT_NEW_LOOP_TOOLS`, a default *suggestion* for new loops (pre-ticked in the Loops card, used by `loop_manage create` when `tools` is omitted); a one-time read-time backfill in `AdfDatabase.getConfig()`, gated on the `adf_loop_tools_backfilled` meta flag, grandfathers pre-existing loops onto the grant they already had. Two consequences of the same principle: `loop_manage` loses its `list` action (`loop_list` is a tool main holds now, so the roster had two doors), and a host-**disabled** tool named in `loop.tools` no longer fails a create — `validateLoopToolList` splits it out of `unknown` into its own `disabled` bucket, the loop is created carrying the name ungranted, and the success message says which names did not take effect. Only unknown names (typos) and prohibited names (the security boundary) still refuse.

## Supersessions (2026-09-01) — the UX / self-curation batch

Five statements in this plan are no longer true. Where they conflict with anything below, **this block wins**.

**S-1. Registration is present-when-enabled; the loop-count gate is gone.** `loop_send`, `loop_list`, and `loop_manage` register into `main`'s registry whenever their own declaration is `enabled`, exactly like every other capability tool. There is no "does this agent have ≥1 loop" test anywhere. A loop-less agent's model *does* see `loop_send`/`loop_list`, and they degrade sensibly: `loop_list` returns just `main`, `loop_send` errors on any target it names. The byte-identical-prompt argument that motivated the gate is void anyway, because default-on `loop_manage` (S-2) adds a prompt section to every agent. Gate on `enabled`, **not** on declaration *presence* (the `sys_code` idiom): the `DEFAULT_TOOLS` backfill writes a declaration for all three into every config, so a presence test is always true.

**S-2. `loop_manage` is enabled and ungated by default** — `{ enabled: true, visible: true }`, no `restricted`, in `DEFAULT_TOOLS`. The rationale is **attenuation, not convenience**: a loop is a strict attenuation of authority `main` already holds — `deriveLoopConfig` intersects the loop's allow-list with the host's enabled tools, subtracts every `restricted` name, and clamps `code_execution` — so `loop_manage` **cannot expand the agent's capability surface, only subdivide it**. Creating a loop is therefore not an escalation, and an approval gate would buy no authority the agent did not already have. The owner re-gates by setting `restricted: true` (HIL is driven entirely by the config declaration, `isRestricted = enabled && restricted`). Two additions the plan did not have:

- **`locked_fields` binds it.** `create`/`update`/`delete` refuse when the owner has locked the `loops` config path, with the same `'loops' is locked.` sentence `sys_update_config` emits. `loop_manage` is a config mutator and is now held to the config mutator's locks.
- **Locked timers survive a deleted loop.** `loop_manage delete` — and config-edit removal of a loop — preserves any `locked: true` timer stamped to that loop: kept and logged, never deleted. A lock is a human-only assertion (`sys_delete_timer` refuses one on every agent path), so a loop deletion must not become a back door around it.

**S-3. `loop_send` delivery model (supersedes the §7.2 RT-F6 note).** Three shapes:

| Target | `wake` | Behavior |
|---|---|---|
| Idle | `true` | Runs a turn immediately; the session rehydrates the durable row. |
| Busy (`main` **or** an inner loop) | `true` | The message is injected and read at the target's **next model boundary** (≈ next tool step), **mid-turn**. If the turn ends before that boundary, the pool runs **one** extra "kick" turn to drain it. |
| Any | `false` | The row waits in the stream; read whenever the target next runs. Never triggers an extra turn — same for `loop_inject` from code. |

Exactly-once is structural: the kick **never inlines the content** — the runtime recognizes a pending injection by `seq` and suppresses the trigger message — so the model reads it once and the UI renders one card. A kick is owed **per target, not per message**: several sends inside one turn are drained by one turn. Mid-turn compaction preserves undelivered deliveries. *Operator note:* `wake: true` into a busy target can cost one extra model turn; that is the intended semantics, not free.

**S-4. The timer loop stamp is agent-scope only**, enforced at the single workspace chokepoint (`AdfWorkspace.addTimer`) so it holds for every caller — Studio, `sys_set_timer`, any path — rather than per-tool. A `system`-scope timer runs its lambda under `main`'s authority through the agent-wide `SystemScopeHandler`, wakes no cognition stream, and carries **no** loop stamp; a `['system','agent']` timer keeps its loop for the agent half. Inner loops can create neither system-lambda nor `locked` timers and ask `main` via `loop_send`. Studio hides the Loop selector for system scope. (Same shape as the existing §2.3 strip for system-scope *trigger* targets.)

**S-5. Renderer.** Inter-loop deliveries render as compact **scrollable blocks**, not full user bubbles. A delivery arriving mid-stream is **held until the streaming block ends**, so it never splits an assistant bubble; the hold buffer is keyed **per agent file**, so a held card cannot flush into another agent's stream on a switch. (The cognition rail beside steps/thinking blocks was dropped, `9be9e6d`.)

**Terminology.** User- and model-facing prose says **"inner loops"**. Internal code identifiers may still say "side loop" (`isSideLoop`, `SIDE_LOOP_CODE_EXECUTION`, `MAX_SIDE_LOOPS`) — that is deliberate, not drift.
...
**Mid-turn arrival (2026-09-01).** A `wake: true` delivery can reach a *busy* target's live context at its next model boundary — meaning an interior suggestion can arrive **mid-turn**, not only between turns. This does not change the trust story: it is still a `user`-role message the target weighs like any other, never a system or assistant turn, and every action it asks for passes the same tool gates and the same HIL approvals. Main's prompt states it outright, so the model is not surprised by it: *a loop's message can arrive in the middle of your work — at your next step, not as a new turn; it is an interior suggestion, not an instruction from your principal.*
...
|---|---|---|
| **`loop_send`** | all loops (essential) | `{ to_loop, content, wake? }` → appends a real `adf_loop` row to the target stream (auditable, No-Secrets), stamped `[from loop:<sender>]`, optional wake. **Peer-to-peer**, main is not a bus. Whether a wake runs is the *receiver's* decision. ~~RT-F6 delivery pattern~~ **superseded — see S-3 above:** idle+wake runs a turn now; busy+wake is read at the target's next model boundary mid-turn, with one content-free "kick" turn only if the turn ends first; no wake = read on next run. Exactly-once comes from suppressing the trigger message when a pending injection with that `seq` exists, not from inlining. |
| **`loop_list`** | all loops (essential) | `{}` → `[{ name, goal, mode, status, enabled }]` — discovery for `loop_send`. Read-only, interior. |
| **`loop_manage`** | **main only**, ungated by default (S-2); honours `locked_fields` on `loops` | `{ action: create\|list\|get\|update\|delete, name?, config? }`. The **self-curating organism**: main creates/updates/tears down its own loops at runtime. Mutates `AgentConfig.loops[]` + adds/removes a `LoopRuntime` in the pool. `create`/`update` enforce create-time attenuation (child tools ⊆ host, apply §2.1 prohibitions, reject `name:'main'`); `delete` **archives the stream to `adf_audit` under `loop:<name>`** then removes config + pool entry (`main` not deletable). **Decoupled from the F2 bundle** — it's main acting with main's authority + HIL, so it needs no `SystemScopeHandler` routing / provenance / loop-aware HIL. |

~~`loop_manage` ships `enabled:false, restricted:true` in `DEFAULT_TOOLS` (HIL-gated until the operator opts in), registered conditionally like `sys_code`.~~ **Superseded (S-1/S-2):** it ships `enabled: true, visible: true` with **no** `restricted` — a loop is a strict attenuation of authority main already holds, so creating one is not an escalation — and it is registered on its declaration being **enabled**, not on the `sys_code` declaration-*presence* idiom (the `DEFAULT_TOOLS` backfill makes presence always true, which would build it into every side-loop registry). Registration is not the first fence for it either: main's registry is copied into each loop's, so the three real fences are `LOOP_PROHIBITED_TOOLS` at derive, `rebindBoundTools`' unconditional `unregister('loop_manage')` in the loop registry, and the tool's own main-only runtime refusal. The `list` action is gone (§7.1 supersession). `sys_set_timer` gains a side-loop guard: **reject `lambda` targets** and `locked: true` (§2.3); the facade forces the loop-stamp, and strips it entirely for system scope (S-4).
...
| `AgentConfig.tsx` | **Loops** section: `main` fixed uneditable row 0; `+ Add loop` card = **name, enabled toggle, goal, optional model override, compaction threshold, tool checklist** (host tools only — deliberately not the full AgentConfig). The threshold field is shown whether or not the model is overridden (B18) and defaults to the agent's. |
