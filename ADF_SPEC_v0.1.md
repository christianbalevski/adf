...
The Agent Document File (`.adf`) is a portable SQLite database that bundles one agent, one primary document, the agent's memory, configuration, message history, files, contacts, scheduled work, and operational records into a single sovereign artifact.
