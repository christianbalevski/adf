...
import type { AgentExecutionEvent, LoopSeqPayload, ResponseMetadataPayload, ToolApprovalRequestPayload } from '../../shared/types/ipc.types'
import { parseLoopSendStamp } from '../../shared/utils/loop-parser'
import { shouldRefreshDocument } from '../../shared/utils/document-target'
import { nanoid } from 'nanoid'
import { findApprovalTargetEntry } from './approval-target'
import { pendingBlockIndexes, pendingToolResultIndexes } from './live-seq'
...
/**
 * Build the agent-event handler.
 *
 * Extracted from the hook so the live transcript path is exercisable without a
 * React tree: the returned function IS what `onAgentEvent` receives, closure
 * (the held loop deliveries) and all. The hook below is now only the
 * subscription wiring.
 */
export function createAgentEventHandler(): (event: AgentExecutionEvent) => void {
  ...
  /**
   * The assistant row that produced the text/thinking blocks currently at the
   * tail has just been persisted — give those entries its seq, the same one
   * `parseLoopToDisplay` gives them on reload. Metadata only; nothing redraws.
   */
  const stampStreamedBlocks = (seq: unknown, loop: string): void => {
    if (typeof seq !== 'number') return
    const store = useAgentStore.getState()
    const log = selectLoopSlice(store, loop).log
    store.stampSeq(pendingBlockIndexes(log), seq, loop)
  }

  /** `metadata.seq` for an entry whose row was persisted before the event. */
  const seqMeta = (payload: { seq?: unknown }): { seq: number } | undefined =>
    typeof payload.seq === 'number' ? { seq: payload.seq } : undefined

  return (event: AgentExecutionEvent) => {
    ...
    switch (event.type) {
      ...
      case 'trigger_message': {
        const payload = event.payload as { content: string; triggerType: string; seq?: number }
        ...
        if (loopSend) {
          deliverLoopMessage({
            ...
            metadata: { ...seqMeta(payload), category: 'loop', fromLoop: loopSend.fromLoop }
        ...
        if (payload.triggerType === 'chat') {
          ...
          agentStore.addLogEntry({
            ...
            content: payload.content,
            timestamp: event.timestamp,
            metadata: { ...seqMeta(payload), triggerType: payload.triggerType }
          }, loop)
        }
        break
      }
      ...
      case 'tool_call_start': {
        const payload = event.payload as { name: string; input: unknown; id?: string; seq?: number }
        // This tool call closes the assistant row: the text/thinking blocks
        // streamed ahead of it belong to the SAME row and share its seq —
        // exactly how parseLoopToDisplay splits that row on reload.
        stampStreamedBlocks(payload.seq, loop)
        ...
        agentStore.addLogEntry({
          ...
          timestamp: event.timestamp,
          metadata: { ...seqMeta(payload), name: payload.name, input: payload.input, ...(payload.id ? { tool_id: payload.id } : {}) }
        }, loop)
        break
      }

      case 'loop_seq': {
        // The tool results shown one-by-one during the batch all landed in one
        // adf_loop row, which only exists now. Pure bookkeeping — no entry is
        // added, removed or redrawn; the transcript just gains the pagination
        // cursor a rehydrated window would have had.
        const payload = event.payload as LoopSeqPayload
        if (typeof payload?.seq !== 'number' || !Array.isArray(payload.toolUseIds)) break
        const log = selectLoopSlice(useAgentStore.getState(), loop).log
        agentStore.stampSeq(pendingToolResultIndexes(log, payload.toolUseIds), payload.seq, loop)
        break
      }
      ...
      case 'tool_call_result': {
        ...
        // cannot, so they no longer trigger an IPC round-trip. Main now derives
        // the write's target path(s) where it can, so a write to some other
        // file costs nothing; when it cannot (shell redirection, sys_code /
        // sys_lambda writes, unknown tool shapes) this still refreshes, because
        // a stale document on screen is worse than an extra read.
        // README.md/document.md also arrive via `document_updated`, but an agent
        // whose document path is neither is covered ONLY here — `file_updated`
        // refreshes open editor tabs, not the document store.
        if (shouldRefreshDocument(payload)) {
      ...
      case 'turn_complete': {
        const turnPayload = event.payload as { targetState?: string; seq?: number }

        // No tool call closed this assistant row, so this is where the
        // text/thinking blocks it streamed get their adf_loop seq.
        stampStreamedBlocks(turnPayload.seq, loop)
      ...
      case 'context_injected': {
        const payload = event.payload as { category: string; content: string; seq?: number }
        ...
        const injectedEntry: AgentLogEntry = {
          ...
          metadata: injectedLoopSend
            ? { ...seqMeta(payload), category: 'loop', fromLoop: injectedLoopSend.fromLoop }
            : { ...seqMeta(payload), category: payload.category }
        }
...
export function useAgentEvents() {
  useEffect(() => {
    ...
    const unsubscribe = api.onAgentEvent(createAgentEventHandler())
