 ...
 * territories, traces, and labels all paint over it. Redraws only when the
 * viewport, size, or theme changes — it's static scenery, not animation.
 */
...
export const FleetGardenLayer = memo(function FleetGardenLayer() {
  ...
  useEffect(() => {
    ...
    // Visible cell range (axial: x = q·COL_W, y = (r + q/2)·ROW_H)
    const wx0 = -tx / zoom - HEX_SIZE
    const wx1 = (rect.width - tx) / zoom + HEX_SIZE
    const wy0 = -ty / zoom - HEX_SIZE
    const wy1 = (rect.height - ty) / zoom + HEX_SIZE
    ...
    const detail = zoom > 0.32 // dew + sprigs only where they resolve
  ...
  }) // no dep array: redraw after every render (transform, resize, theme)

  // Same slot as HexBackground (behind all nodes), one layer above it so
  // growth tints sit on top of the lattice fills
  return <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: -9 }} />
})
