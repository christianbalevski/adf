...
import type { AdapterRegistration, AdapterCredentialFileInfo, TrackedDirEntry } from '../../../shared/types/ipc.types'
...
interface AdapterCredentialPanelProps {
  adapter: AdapterRegistration
  registryEntry?: AdapterRegistryEntry
  onAdapterUpdate: (patch: Partial<AdapterRegistration>) => void
}

/**
 * Credential management panel for a channel adapter.
 ...
 */
export function AdapterCredentialPanel({ adapter, registryEntry, onAdapterUpdate }: AdapterCredentialPanelProps) {
  const storageMode = adapter.credentialStorage ?? 'app'

  // Env keys from registry
  const allEnvKeys = registryEntry
    ? [...(registryEntry.requiredEnvKeys ?? []), ...(registryEntry.optionalEnvKeys ?? [])]
    : (adapter.env ?? []).map((e) => e.key).filter(Boolean)
  ...
  const removeEnvVar = (index: number) => {
    onAdapterUpdate({ env: (adapter.env ?? []).filter((_, i) => i !== index) })
  }

  // --- Per-agent (ADF) credential state ---
  const [adfFiles, setAdfFiles] = useState<AdapterCredentialFileInfo[]>([])
  const [manualAdfFiles, setManualAdfFiles] = useState<AdapterCredentialFileInfo[]>([])
  ...
  const loadAdfFiles = useCallback(async () => {
    ...
    try {
      const result = await window.adfApi?.listAdapterCredentialFiles({
  ...
  const loadCredentialsForFile = async (filePath: string) => {
    try {
      const result = await window.adfApi?.getAdapterCredentials({
        filePath,
        adapterType: adapter.type
      })
  ...
  const handleSaveAdfCredentials = async (filePath: string) => {
    ...
    try {
      ...
      // Attach the adapter config to the ADF (idempotent)
      const attachResult = await window.adfApi?.attachAdapter({
        filePath,
        adapterType: adapter.type,
        config: {
          enabled: true,
      ...
      if (attachResult && !attachResult.success) {
        setSaveStatus((prev) => ({ ...prev, [filePath]: attachResult.error ?? 'Failed to attach adapter' }))
        setSavingFor(null)
        return
      }
      ...
      for (const [envKey, value] of keysToSave) {
        const result = await window.adfApi?.setAdapterCredential({
          filePath,
          adapterType: adapter.type,
          envKey,
          value
      ...
      const updateFile = (f: AdapterCredentialFileInfo) =>
        f.filePath === filePath
  ...
  const handleSelectAdf = (rowId: number, filePath: string) => {
    ...
    const newFile: AdapterCredentialFileInfo = {
      filePath,
  ...
  const handleRemoveAdfFile = async (filePath: string) => {
    // Detach: remove adapter config + credentials from the ADF
    await window.adfApi?.detachAdapter({
      filePath,
      adapterType: adapter.type
    })
  ...
  const handleStorageModeChange = (mode: 'app' | 'agent') => {
    onAdapterUpdate({ credentialStorage: mode })
  }
  ...
  if (allEnvKeys.length === 0 && !registryEntry) {
    return (
      <p className="text-xs text-neutral-500 italic px-3 py-2">
        This adapter requires no credentials.
      </p>
    )
  }
    ...
    <div className="px-3 py-2 space-y-3">
      ...
      <div>
        ...
        <p className="text-[9px] text-neutral-400 dark:text-neutral-500 mt-1">
          {storageMode === 'app'
            ? 'Credentials stored in app settings, shared by all agents using this adapter.'
