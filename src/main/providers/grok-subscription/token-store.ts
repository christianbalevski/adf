...
const DIR_NAME = 'grok-subscription'
...
function encryptValue(value: string): string {
  ...
  console.warn('[Grok Auth] safeStorage unavailable, storing tokens in plaintext')
  return value
}
...
function decryptValue(raw: string): string | null {
  if (raw.startsWith(SAFE_STORAGE_PREFIX)) {
    const safeStorage = getSafeStorage()
    if (!safeStorage?.isEncryptionAvailable()) {
      console.warn('[Grok Auth] safeStorage unavailable, cannot decrypt tokens')
      return null
    }
    try {
      ...
      console.warn('[Grok Auth] Failed to decrypt tokens:', err)
      return null
    }
  }
  return raw
...
export function readTokens(): TokenSet | null {
  ...
  try {
    ...
    console.warn('[Grok Auth] Failed to read tokens:', err)
    return null
  }
}
