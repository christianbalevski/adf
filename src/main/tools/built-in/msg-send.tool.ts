...
const InputSchema = z.object({
  ...
  content_type: z
    .string()
    .optional()
    .describe('MIME type of content when it is not plain text/markdown. "application/vnd.adf.form+json": structured form (JSON: { id, title?, questions: [{ id, text, type: "choice"|"multi"|"text", options?: [{id, label}] }], render? }) — RICH rendering on telegram only right now: a single choice/multi question becomes a native poll, all-choice forms become one message with a combined keyboard, forms with text questions send one message per question. An explicit render: "poll"|"compact"|"per_question" is a strict contract — if the form shape does not satisfy it the send FAILS with the reason (omit render for auto selection). Answers return as threaded replies with form_id/question_id/answer_value in source_context. Other adapters render forms as a plain-text questionnaire, so for them just ask in a normal message instead. "text/html": HTML body — honored by email (full HTML) and telegram (limited tag subset); other adapters convert it to plain text, so prefer markdown there.'),
  subject: z
    .string()
    .optional()
