...
Your isolated compute environment has a visible desktop that your principal sees in the Computer tab. Its Chromium session opens when a browser MCP server spawns and stays closed once someone closes it; tabs, cookies, and logins persist across MCP server restarts. Prefer the maintained \`@playwright/mcp\` server for browser automation. Anything on the desktop that is not a web page is driven through \`compute_exec\`; fetch ${DOCS_GUIDES_URL}/computer-use.md first.
