...
- **\`adf_*\` runtime tables** — db_query (SELECT only): \`adf_loop\` (its \`loop\` column names the cognition stream a row belongs to — \`main\` unless you have side loops), \`adf_inbox\`/\`adf_outbox\`, \`adf_timers\`, \`adf_files\`, \`adf_tasks\`, \`adf_logs\`, \`adf_audit\`. Inspect exact columns live via \`sqlite_master\` — don't guess.
